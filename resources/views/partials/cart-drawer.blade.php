...
>
    ...
    >
        ...
        <div x-show="$store.cart.items.length > 0" class="border-t border-[var(--color-paper-3)] px-5 py-4">
            ...
            <button @click="$store.cart.checkoutQris()"
                class="mt-2 flex w-full items-center justify-center gap-2 rounded-[var(--radius-xl)] border-2 border-[var(--color-accent)] py-3 text-sm font-semibold text-[var(--color-accent)] transition-all hover:bg-[var(--color-accent-light)] active:scale-[0.98]">
            ...
            <p class="mt-2 text-center text-[11px] text-[var(--color-ink-3)]">
                QRIS: bayar online — pesanan langsung diproses. WhatsApp: konfirmasi via chat, bayar di tempat / transfer.
            </p>
        </div>
    </div>
</div>
