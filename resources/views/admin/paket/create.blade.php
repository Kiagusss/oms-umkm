...
@section('title', 'Tambah Paket — Admin Pempek')

@section('content')
@include('admin.partials.page-header', [
    'title' => 'Tambah Paket',
    'description' => 'Lengkapi data paket.',
...
<div class="max-w-3xl rounded-[var(--radius-xl)] border border-[var(--color-paper-3)] bg-white p-6 sm:p-8">
    ...
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Isi Paket</label>
            <textarea name="items" rows="4" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">{{ old('items', '') }}</textarea>
</div>
    <div class="grid gap-6 sm:grid-cols-2"><div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Harga (Rp)</label>
            <input type="number" name="price" min="0" value="{{ old('price', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Harga Coret (Rp)</label>
            <input type="number" name="original_price" min="0" value="{{ old('original_price', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Badge</label>
            <input type="text" name="badge" value="{{ old('badge', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
    </div>
    <div>
