...
@section('title', 'Tambah Voucher — Admin Pempek')

@section('content')
@include('admin.partials.page-header', [
    'title' => 'Tambah Voucher',
    'description' => 'Buat voucher diskon baru untuk kasir (POS).',
...
<div class="max-w-3xl rounded-[var(--radius-xl)] border border-[var(--color-paper-3)] bg-white p-6 sm:p-8">
    <form method="POST" action="{{ route('admin.voucher.store') }}" class="space-y-6">
        @csrf

        <div class="grid gap-6 sm:grid-cols-2">
            <div>
                <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Kode Voucher *</label>
                <input type="text" name="code" required value="{{ old('code', '') }}" placeholder="HEM10" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm uppercase">
            ...
            <div>
                ...
                <select name="type" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
                    ...
                    <option value="fixed" @selected(old('type', 'percentage') === 'fixed')>Nominal (Rp)</option>
                </select>
            ...
            <div>
                ...
                <input type="number" name="value" required step="0.01" min="0.01" value="{{ old('value', '') }}" placeholder="10 atau 5000" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input type="number" name="min_order" step="0.01" min="0" value="{{ old('min_order', '') }}" placeholder="Kosong = tanpa minimal" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input type="number" name="max_uses" min="1" value="{{ old('max_uses', '') }}" placeholder="Kosong = tanpa batas" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input type="datetime-local" name="valid_from" value="{{ old('valid_from', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
            ...
            <div>
                ...
                <input type="datetime-local" name="valid_until" value="{{ old('valid_until', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
                @error('valid_until')<p class="mt-1 text-xs text-[var(--color-danger)]">{{ $message }}</p>@enderror
            </div>
            <div class="sm:col-span-2">
                <label class="flex cursor-pointer items-center gap-3">
                    <input type="checkbox" name="is_active" value="1" @checked(old('is_active', true)) class="h-4 w-4 rounded border-[var(--color-paper-3)] text-[var(--color-accent)]">
