...
@section('title', 'Tambah Banner — Admin Pempek')

@section('content')
@include('admin.partials.page-header', [
    'title' => 'Tambah Banner',
    'description' => 'Lengkapi data banner.',
...
<div class="max-w-3xl rounded-[var(--radius-xl)] border border-[var(--color-paper-3)] bg-white p-6 sm:p-8">
    <form method="POST" action="{{ route('admin.banner.store') }}" class="space-y-6">
        @csrf
...
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Subjudul</label>
            <input type="text" name="subtitle" value="{{ old('subtitle', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Teks Tombol</label>
            <input type="text" name="button_text" value="{{ old('button_text', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">Link Tombol</label>
            <input type="text" name="button_link" value="{{ old('button_link', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
    <label class="mb-1 block text-sm font-semibold text-[var(--color-ink)]">URL Gambar Latar</label>
            <input type="text" name="background_image" required value="{{ old('background_image', '') }}" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
</div>
<div>
