...
@section('title', 'Buat Resep Baru — Admin Pempek')

@section('content')
@include('admin.partials.page-header', [
    'title' => 'Buat Resep & Bill of Materials',
    'description' => 'Tentukan komposisi bahan untuk menghitung HPP otomatis per porsi.',
...
<div class="max-w-4xl rounded-[var(--radius-xl)] border border-[var(--color-paper-3)] bg-white p-6 sm:p-8"
    ...
    <form method="POST" action="{{ route('admin.resep.store') }}" class="space-y-6">
        @csrf
                ...
                <select name="variant_id" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">
                    ...
                    @foreach($products as $p)
                        @foreach($p->variants as $v)
                            <option value="{{ $v->id }}" @selected(old('variant_id') == $v->id)>
                                {{ $p->name }} — {{ $v->name }}
                            </option>
                        @endforeach
                    @endforeach
        ...
        <div>
            ...
            <textarea name="instructions" rows="3" class="w-full rounded-[var(--radius-md)] border border-[var(--color-paper-3)] px-3 py-2 text-sm">{{ old('instructions') }}</textarea>
        </div>
        ...
        <div class="pt-4 border-t border-slate-100 flex justify-end gap-3">
            ...
            <button type="submit" class="rounded-[var(--radius-md)] bg-[var(--color-accent)] px-5 py-2 text-sm font-semibold text-white hover:bg-[var(--color-accent-hover)]">Simpan & Hitung HPP</button>
        </div>
    </form>
</div>
@endsection
